...
# We declare globals here and initialize them inside main().
root = None
command_value = None
num_value = None
ev_value = None
...
slot_value = None
ev_motor_value = None
level_remove_value = None
tfield = None


# Parking Lot class (unchanged logic; still references GUI globals)
class ParkingLot:
    def __init__(self):
        self.capacity = 0
        self.evCapacity = 0
        ...
        if (self.numOfOccupiedEvSlots < self.evCapacity) or (
            ...
            if ev == 1:
                ...
                if self.numOfOccupiedSlots < self.capacity:
                    ...
                    if motor == 1:
                        # Note: baseline has type mixups; preserved for now
    ...
    def status(self):
        # Uses global tfield (GUI); safe only when main() has initialized it
        ...
        for i in range(len(self.slots)):
            if self.slots[i] != -1:
                output = (
                    ...
                    + str(self.level)
                    + "\t"
                    + str(self.slots[i].regnum)
                    + "\t\t"
                    + str(self.slots[i].color)
                    + "\t\t"
                    + str(self.slots[i].make)
                    + "\t\t"
                    + str(self.slots[i].model)
                    + "\n"
                )
                tfield.insert(tk.INSERT, output)
        ...
        for i in range(len(self.evSlots)):
            if self.evSlots[i] != -1:
                output = (
                    ...
                    + str(self.level)
                    + "\t"
                    + str(self.evSlots[i].regnum)
                    + "\t\t"
                    + str(self.evSlots[i].color)
                    + "\t\t"
                    + str(self.evSlots[i].make)
                    + "\t\t"
                    + str(self.evSlots[i].model)
                    + "\n"
                )
                tfield.insert(tk.INSERT, output)
    ...
    # NOTE: The next two functions have baseline bugs (param names vs variables).
    # We leave them untouched in Step 7; they will be fixed in later steps.
    def getSlotNumFromMakeEv(self, color):
        slotnums = []
        for i in range(len(self.evSlots)):
            if self.evSlots[i] == -1:
                continue
            if self.evSlots[i].make == make:  # noqa: F821 (baseline bug)
                slotnums.append(str(i + 1))
        return slotnums
    ...
    def getSlotNumFromModelEv(self, color):
        ...
        for i in range(len(self.evSlots)):
            if self.evSlots[i] == -1:
                continue
            if self.evSlots[i].model == model:  # noqa: F821 (baseline bug)
                slotnums.append(str(i + 1))
        return slotnums
    ...
    def slotNumByColor(self):
        ...
        tfield.insert(tk.INSERT, "Identified slots: " + ", ".join(slotnums) + "\n")
        tfield.insert(
            tk.INSERT, "Identified slots (EV): " + ", ".join(slotnums2) + "\n"
        )
    ...
    def regNumByColor(self):
        ...
        tfield.insert(
            tk.INSERT, "Registation Numbers: " + ", ".join(regnums) + "\n"
        )
        tfield.insert(
            tk.INSERT, "Registation Numbers (EV): " + ", ".join(regnums2) + "\n"
        )

    def makeLot(self):
        self.createParkingLot(
            int(num_value.get()), int(ev_value.get()), int(level_value.get())
        )
        output = (
    ...
    def removeCar(self):
        ...
        if status:
            tfield.insert(
                tk.INSERT, f"Slot number {str(slot_value.get())} is free\n"
            )
        else:
            tfield.insert(
                tk.INSERT,
                "Unable to remove a car from slot: " + slot_value.get() + "\n",
            )
...
def main() -> None:
    ...
    global slot1_value, slot2_value, reg1_value, slot_value, ev_motor_value
    global level_remove_value, tfield
    ...
    parkMakeBtn = tk.Button(
        root,
        command=parkinglot.makeLot,
    ...
    slotRegBtn = tk.Button(
        root,
        command=parkinglot.slotNumByReg,
    ...
    slotColorBtn = tk.Button(
        root,
        command=parkinglot.slotNumByColor,
    ...
    regColorBtn = tk.Button(
        root,
        command=parkinglot.regNumByColor,
    ...
    chargeStatusBtn = tk.Button(
        root,
        command=parkinglot.chargeStatus,
